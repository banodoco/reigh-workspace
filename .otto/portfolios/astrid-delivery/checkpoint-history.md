...
Normally use 6–10 short lines. For a genuinely unchanged interval, a compact entry naming the same task/source, waiting event/owner and previous-action state is enough. Still perform the project's Luna check, but accept sparse evidence when little legitimately happened; do not demand more output or nudge an unchanged assignment merely to fill the entry. A missing monitoring report is not a new acceptance gate. Never copy the entire project plan or repeat unchanged historical evidence. No new tests are launched merely to populate the history.

## Judgment and next-check feedback

After reading the project reports and comparing them with the original tasklists/dependencies, Astra states its judgment in the existing orchestrator thread. Append that exact judgment directly under Astra low's clerical exception, or batch its recording with the status update through an existing worker. Record in each entry: continue / unblock / redirect / await named event / hold affected lane; reason; assigned manager/action; evidence expected; next meaningful check or event. Wait until the investigator's observation is finished; then the coordinator or one assigned recorder holds sole write custody. Do not create a separate agent turn for every tiny append. No investigator may invent Astra's judgment. Until supplied, the decision is explicitly **pending**; an observation is not an accepted gate.
...
Astra low or one batched recorder maintains a short row per active project in status.md: original task/gate, latest observed change, Astra disposition, owner/next action and link to the latest history entry. Use portable relative links within the receiving workspace when possible; actual local history links are created at bootstrap, not shipped as broken public links. Older rows are not a competing history—the per-project file owns chronology. The hourly user update highlights material changes, discrepancies and actions; it need not reproduce every entry.
