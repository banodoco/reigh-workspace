...
Use `python3 scripts/open-manager.py PROJECT_ID --workspace /absolute/workspace` to inspect a launch command; it is **dry-run by default**. Once you have checked there is no existing live owner, add `--execute` to open one Terminal window. The helper reads the coordinator binding from the active project's run.yaml, falling back to the frozen handover only for initial readiness if the active root is absent. It makes no permission-policy override. Projects are `database`, `generation`, `visualization`, `render`, and `ephemeral`; UE is intentionally not a new-launch option.
...
Start with at most **two concurrently active implementation workers portfolio-wide**, plus managers and a required reviewer where appropriate. This is a conservative scheduling default, not a user budget or fixed architectural rule. The orchestrator may adjust it with a short rationale based on disjoint write sets, machine capacity and observed throughput. Account for workers across every manager, not just one thread's native limit. No manager should spawn workers just to appear busy; reviewers remain leaves, except a separately explicit user-authorized research hierarchy.
