...
- [ ] **Per-sprint gates credible** — every Sprint N row in §4 has concrete exit criteria, and predecessor exit criteria are sufficient to start the next sprint. **[Owner: TBD-<engineering-lead> | Date: pending]** — Blocked on engineering lead sign-off. Sprint 0A produced a reviewable gate structure, but final credibility approval is a human leadership decision.
- [ ] **Pre-Sprint-4 dry run plan staffed** — owner named (VibeComfy maintainer + reigh-worker adapter author per §3A "Wan 2.2 VACE cocktail"); reference output identified; dry-run pod budgeted. **[Owner: TBD-<VibeComfy-maintainer> / TBD-<adapter-author> | Date: pending]** — Blocked on naming the VibeComfy maintainer and adapter author.
- [ ] **Rollback control plane has an owner** — Sprint 11 canary owner has committed to runtime selector rollback plus draft PRs for durable follow-up before promotion. **[Owner: TBD-<canary-owner> | Date: pending]** — Blocked on assigning an operational owner with rollback, monitoring, and follow-up responsibility.
- [ ] **Migration-blocking §8A checks identified** — §8A rows are split into migration-blocking vs cleanup-only; only migration-blocking rows can gate Sprint 12 dual-executor hardening. **[Owner: TBD-<engineering-lead> | Date: pending]** — Blocked on engineering lead approval of the migration-blocking classification.
...
- [ ] **Pod disk-size change approved and owned** — Sprint 0C owns the 200 GB disk-size change or an explicit owner/date for it, and measures first dual-stack pod boot/download behavior. If the code change lands before Sprint 0, regenerate affected baselines. **[Owner: TBD-<pod-infra-owner> | Date: pending]** — Blocked on infrastructure owner sign-off and budgeting.
- [ ] **Custom-node lockfile audited** — every template `template_routing.py` can select has its `READY_REQUIREMENTS.custom_nodes` represented in `vibecomfy/custom_nodes.lock`. **[Owner: TBD-<VibeComfy-maintainer> | Date: pending]** — Blocked on VibeComfy maintainer audit.
- [ ] **Capacity/security owners named for canary readiness** — not a Sprint 1 blocker, but Sprint 10 must have named owners for capacity/cost plan, artifact retention/privacy, selector ACLs, and alert routing. **[Owner: TBD-<security-lead> | Date: pending]** — Blocked on security/capacity owner assignment.
