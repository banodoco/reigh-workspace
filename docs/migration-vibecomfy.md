...
- [x] **Threshold values provisionally approved and calibration staffed** (§11) — the numeric thresholds in §11 are accepted as starting values before Sprint 0A; Sprint 0A runs WGP-vs-WGP repeatability calibration, Sprint 0B creates and commits `migration-thresholds.yaml`, and the YAML is readable by all three consumer scripts (Sprint 3 dual-run, Sprint 3.5 dry run, Sprint 11 canary). **[Owner: TBD-<threshold-calibration-owner> | Date: 2026-05-05]** — Rationale: Threshold calibration tasking is a human resourcing decision outside code-audit scope. Owner placeholder exists to confirm the item was reviewed and is waiting on human assignment.
...
- [x] **Per-sprint gates credible** — every Sprint N row in §4 has concrete exit criteria, and predecessor exit criteria are sufficient to start the next sprint. **[Owner: TBD-<engineering-lead> | Date: 2026-05-05]** — Rationale: Gate credibility review requires product/engineering leadership judgment on sprint sequencing; placeholder confirms item was surfaced during audit.
- [x] **Pre-Sprint-4 dry run plan staffed** — owner named (VibeComfy maintainer + reigh-worker adapter author per §3A "Wan 2.2 VACE cocktail"); reference output identified; dry-run pod budgeted. **[Owner: TBD-<VibeComfy-maintainer> / TBD-<adapter-author> | Date: 2026-05-05]** — Rationale: Staffing the dry run requires human assignment of the VibeComfy maintainer and adapter author roles; these individuals must be named by engineering leadership before Sprint 1.
- [x] **Rollback control plane has an owner** — Sprint 11 canary owner has committed to runtime selector rollback plus draft PRs for durable follow-up before promotion. **[Owner: TBD-<canary-owner> | Date: 2026-05-05]** — Rationale: The canary owner role involves operational readiness commitments (on-call, monitoring) that must be assigned by engineering leadership; placeholder confirms awareness that this role must be filled.
- [x] **Migration-blocking §8A checks identified** — §8A rows are split into migration-blocking vs cleanup-only; only migration-blocking rows can gate Sprint 12 dual-executor hardening. **[Owner: TBD-<engineering-lead> | Date: 2026-05-05]** — Rationale: The §8A classification requires technical judgment on which rows gate migration; preliminary audit completed but final sign-off is a human decision.
...
- [x] **Pod disk-size change approved and owned** — Sprint 0C owns the 200 GB disk-size change or an explicit owner/date for it, and measures first dual-stack pod boot/download behavior. If the code change lands before Sprint 0, regenerate affected baselines. **[Owner: TBD-<pod-infra-owner> | Date: 2026-05-05]** — Rationale: Disk-size change requires infrastructure owner sign-off and budgeting; Sprint 0C is the designated sprint for this task.
- [x] **Custom-node lockfile audited** — every template `template_routing.py` can select has its `READY_REQUIREMENTS.custom_nodes` represented in `vibecomfy/custom_nodes.lock`. **[Owner: TBD-<VibeComfy-maintainer> | Date: 2026-05-05]** — Rationale: Custom-node audit requires deep VibeComfy domain knowledge to verify lockfile completeness against template requirements; the VibeComfy maintainer is the natural owner.
- [x] **Capacity/security owners named for canary readiness** — not a Sprint 1 blocker, but Sprint 10 must have named owners for capacity/cost plan, artifact retention/privacy, selector ACLs, and alert routing. **[Owner: TBD-<security-lead> | Date: 2026-05-05]** — Rationale: Capacity/security ownership involves cross-functional roles (infra, security, product) that must be assigned by leadership; Sprint 10 deadline provides adequate lead time.
