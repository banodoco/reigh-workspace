...
**Status (2026-05-05):** Sprint 0B has produced the executable threshold and route source of truth: `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`, `schema_version: 1`). All dual-run scripts (Sprint 3), the Sprint 3.5 dry run, and Sprint 11 canary triggers MUST read thresholds and route statuses from that YAML through the dual-run-compare threshold loader, not from copied values in this document. This table remains the human-readable mirror of the metric rows. Changing a threshold or route status requires updating the YAML, rerunning `python -m scripts.dual_run_compare.check_thresholds --strict`, and updating this section in the same PR.
