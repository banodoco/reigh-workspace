...
Companion document: [Live Validation Plan: reigh-worker VibeComfy Migration](./migration-vibecomfy-live-validation.md). That document owns the RunPod/cloud validation strategy, worker live-test strategy, Astrid semantic grading, and evidence package required before canary.
...
| `travel_segment` | YES (USED-INDIRECTLY — child of `travel_orchestrator`) | NEW for Wan family (Q17 closed); ADAPT for LTX family | Wan family: `ready_templates/video/wanvideo_wrapper_22_14b_vace_cocktail.py` (the same Sprint 4 NEW template — its `start_image`/`end_image` SetNodes mirror `wanvideo_wrapper_13b_vace.py:236-279`). LTX no-control first/last family: `ready_templates/video/ltx2_3_lightricks_first_last_parity.py` with named workflow inputs. LTX raw-video/control variants stay on their dedicated templates. **Per-combination wiring: §3A "Travel-segment configuration matrix" (13 rows).** | `_derive_model_family` at `source/task_handlers/travel/orchestrator.py:38-49` chooses Wan vs LTX. `_apply_video_source_continuation` (`task_registry.py:1106-1132`) writes `video_source` into params — maps to a `VHS_LoadVideo` source-node patch on the chosen template. | DepthAnythingV2 inline (Wan-VACE) or pre-processed Canny/Pose/Flow per §3A. Uni3C via §3A nodes when `_apply_uni3c_config` (`task_registry.py:1326-1328`) is active. | Wan: `WanVideoLoraSelectMulti` patch; LTX no-control parity template patches named inputs; LTX control templates keep their existing LoRA/control wiring. | E / Sprint 6 (Wan path blocked on Sprint 4 NEW template) |
...
- LTX-family no-control first/last travel segment: `ready_templates/video/ltx2_3_lightricks_first_last_parity.py`. The worker scratchpad patches named VibeWorkflow inputs instead of Runexx node IDs. Raw-video guide and IC-LoRA control variants intentionally remain on their dedicated LTX templates.
...
| 7 | ltx / `ltx-2.3` | i2v | none (only kind allowed) | LTX 2.3 distilled LoRA in template ± user `loras` | `ready_templates/video/ltx2_3_lightricks_first_last_parity.py` | Named `start_image_path` / `end_image_path` inputs plus `width`, `height`, `frames`, `fps`, prompts, seeds, and strengths patched by worker scratchpad; prefix-video continuation remains outside this no-control first/last contract. | None | Named-input parity template; no Runexx node-id patching. | LOCAL CONTRACT VALID; RunPod validation orchestrator-owned | `modelCapabilities.ts:107-128` × `travel_guidance.py:65-67` × local batches 1-7 |
| 8 | ltx / `ltx-2.3-fast` | i2v | none | distilled-fast LoRA in template ± user `loras` | same as #7 | same as #7 | None | same as #7 | LOCAL CONTRACT VALID; RunPod validation orchestrator-owned | `modelCapabilities.ts:130-160` × `travel_guidance.py:64-66` × local batches 1-7 |
| 9 | ltx / `ltx-2.3-fast` | i2v | ltx_control:video | same as #8 + auto-injected union IC-LoRA (`travel_guidance.py:30-37,305-325`) | same as #7 | same as #7 + a control reference video → `VHS_LoadVideo` (second slot) | Built into LTX 2.3 distilled controlnet path; control video is the rail itself (no preprocessor) | LTX `LoraLoaderModelOnly` chain extended by IC-LoRA injection (§3A LoRA stacking) | BLOCKED (control-capable LTX template unverified) | `travel_guidance.py:27,305-325,331-336` × `modelCapabilities.ts:49` |
| 10 | ltx / `ltx-2.3-fast` | i2v | ltx_control:pose | same as #9 + cameraman/pose IC-LoRA | same as #7 | same as #9 | Pre-process: `PoseBodyFaceVideoAnnotator` (§3A "Control rails"); guide via `VHS_LoadVideo` | same as #9 | BLOCKED (control-capable LTX template unverified) | `travel_guidance.py:27,297-303` |
| 11 | ltx / `ltx-2.3-fast` | i2v | ltx_control:depth | same as #9 | same as #7 | same as #9 | In-workflow Depth (when template carries it) or pre-process via `DepthV2VideoAnnotator` (§3A) | same as #9 | BLOCKED (control-capable LTX template unverified) | `travel_guidance.py:27,297-303` |
| 12 | ltx / `ltx-2.3-fast` | i2v | ltx_control:canny | same as #9 | same as #7 | same as #9 | Pre-process: `CannyVideoAnnotator` (§3A) | same as #9 | BLOCKED (control-capable LTX template unverified) | `travel_guidance.py:27,297-303` |
| 13 | ltx / `ltx-2.3-fast` | i2v | ltx_control:cameraman | same as #9 + cameraman IC-LoRA (`travel_guidance.py:33-37`) | same as #7 | same as #9 | Cameraman is a metadata-driven control; same `VHS_LoadVideo` slot as `ltx_control:video` | same as #9 | BLOCKED (control-capable LTX template unverified) | `travel_guidance.py:33-37` |
...
**Matrix-level disposition.** Rows 1-6 depend on the Sprint 4 NEW Wan 2.2 VACE cocktail template (§3A "Wan 2.2 VACE cocktail — Q17 verdict"). Rows 7-8 now resolve to `ltx2_3_lightricks_first_last_parity.py` and are locally contract/test validated; fresh RunPod evidence for that template is intentionally orchestrator-owned after local gates. Rows 9-13 are BLOCKED until a real LTX control-capable path is verified for a full-length control guide; they must remain WGP-only/blocked in reports rather than relying on a first/last-only seam. Four vestigial axes are flagged for §8A cleanup.
...
| Sprint 0B: Thresholds and golden corpus | 1 week | Produce comparison oracles consumed by later sprints. | `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`); WGP self-repeatability report at `reigh-worker/scripts/dual_run_compare/reports/wgp-self-repeat-0b-2026-05-05-deferral.{json,md}`; route-keyed WGP golden corpus for Cohort A/B and representative Cohort E routes; lightweight product-contract fixtures for `video_enhance`, `image-upscale`, `animate_character`, and `flux_klein_edit`; live-validation doc updated to same route/threshold version. | Threshold YAML is committed and read by `python -m scripts.dual_run_compare.check_thresholds --strict`; current route statuses are `deferred_pending_sprint_0c_disk` until a later sprint needs measured WGP drift; corpus is route-keyed, not only task-type-keyed; non-RayWorker active routes have contract fixtures or explicit deferral rationale. | `reigh-worker` with owning runtime input | Subjective comparison gates remain possible or active non-RayWorker routes lack regression coverage. | Keep route WGP-only/current-owner-only or widen thresholds only with calibration notes in the YAML/report. |
...
| Sprint 11: Production canary by route cohort | 2 weeks | Promote RayWorker routes sequentially by selector while monitoring active non-RayWorker routes for shared-contract regressions. | Runtime selector flips; canary runbook; cohort dashboards; shadow/dual-run reports evaluated against `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`); rollback PRs; smoke/alert watch for `video_enhance`, `image-upscale`, `animate_character`, and `flux_klein_edit`. | Cohort A holds 48h before B, B holds before E; emergency rollback meets SLO; shadow checks have no completion, billing, upload, or user-visible side effects; WGP remains selectable; active non-RayWorker routes remain green or canary pauses. Rollback triggers use the executable YAML threshold source of truth, not copied prose thresholds. | `reigh-worker` + `reigh-worker-orchestrator` + owning runtimes | Selector promotes a route before all variants are ready or shared changes regress Cloud Worker/external routes. | Flip affected RayWorker route keys back to WGP; leave non-RayWorker routes on their current owner/runtime. |
...
Rollback triggers are defined by `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`, mirrored in §11) and Sprint 0 baselines:

| Trigger (concrete threshold per YAML/§11) | Action |
| --- | --- |
...
| Q2 | Does the Wan 2.2 HIGH->LOW two-stage sampler reproduce WGP closely enough? | Sprint 3.5 | If YAML version `0B-2026-05-05` video thresholds fail, keep Wan-family VACE routes WGP-only. |
...
**Status (2026-05-05):** Sprint 0B has produced the executable threshold and route source of truth: `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`, `schema_version: 1`). The YAML exposes row-granular `metric_keys`, metric `defaults`, and route entries. All dual-run scripts (Sprint 3), the Sprint 3.5 dry run, and Sprint 11 canary triggers MUST read thresholds and route statuses from that YAML through the dual-run-compare threshold loader, not from copied values in this document. This table remains the human-readable mirror of the metric rows. Changing a threshold or route status requires updating the YAML, rerunning `python -m scripts.dual_run_compare.check_thresholds --strict`, and updating this section in the same PR.

**Current calibration status:** WGP repeatability evidence for version `0B-2026-05-05` is committed at `reigh-worker/scripts/dual_run_compare/reports/wgp-self-repeat-0b-2026-05-05-deferral.json` and `.md`. All 14 route keys are currently marked `deferred_pending_sprint_0c_disk` because the report contains no paired WGP-vs-WGP metric observations. This is not a RunPod-access failure: live RunPod lifecycle was separately verified on 2026-05-05 with pod `f9s5vqk15gux9d`. After user correction, WGP remains the trusted control; run paired WGP calibration only when a later sprint needs fresh measured drift to promote route statuses.
...
- `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` — machine-readable source of truth for the table above, with top-level `metric_keys`, metric `defaults`, route entries, calibration statuses, and route notes (`version: 0B-2026-05-05`). Calibration runs same backend vs same backend, fixed seed, target GPU/profile combinations, and the selected route-key corpus. If WGP self-drift exceeds a starting threshold, the threshold must be widened with explicit rationale or the route must stay WGP-only/deferred until a deterministic comparison method exists.
- `reigh-worker/scripts/dual_run_compare/route_keys.py` — canonical route-key implementation. Direct Cohort A/B product routes use direct keys; Cohort B edit variants add dimensions when variant dimensions are present; Cohort E keys include task type, model family, guidance kind, continuity case, and profile.
- `reigh-worker/scripts/dual_run_compare/golden/<route_key>/manifest.json` — WGP-side golden corpus manifest for every selector route intended for promotion, not just every task type. Seed fixture references live under `reigh-worker/scripts/dual_run_compare/fixtures/golden_seed_payloads/`. Cohort E keys include `(task_type, model_name/model_family, guidance_kind, continuity_case, profile)`; Cohort B keys include edit model variant, mask/annotation/style-reference cases, and relevant profile. Sprint 3 dual-run compares Comfy outputs against this corpus + a fresh WGP run.
- `reigh-worker/scripts/dual_run_compare/reports/wgp-self-repeat-0b-2026-05-05-deferral.{json,md}` — route-keyed WGP repeatability deferral evidence for YAML version `0B-2026-05-05`, including attempted command shapes, blocker, calibration status, and next action for every route.
...
- Sprint 3 exit criteria: "Comparison report fails (red) for landed routes if any metric in `migration-thresholds.yaml` version `0B-2026-05-05` is exceeded; not-yet-routed rows are classified rather than blocking Sprint 3."
- Sprint 3.5 dry-run gate: "The YAML video metrics, including `video_phash_mean` and `video_phash_p95`, pass on the Wan 2.2 VACE cocktail single-shot dry run; otherwise fall back per §6 (keep Wan VACE on WGP indefinitely; remainder of migration proceeds)."
- Sprint 11 rollback trigger: auto-flip cohort to `wgp` when the YAML latency, OOM, or canary-divergence metrics breach their configured thresholds from isolated shadow runs. If no safe shadow path exists, output divergence blocks further promotion and is reviewed offline.
...
- [x] **Threshold values provisionally approved and calibration staffed** (§11) — the numeric thresholds in §11 are accepted as starting values before Sprint 0A; Sprint 0B has created `reigh-worker/scripts/dual_run_compare/migration-thresholds.yaml` (`version: 0B-2026-05-05`) as the executable threshold source of truth for Sprint 3 dual-run, Sprint 3.5 dry run, and Sprint 11 canary consumers. **[Owner: Peter O'Malley, interim threshold-calibration owner | Date: 2026-05-05]** — Approved to proceed by chain owner instruction; Sprint 0C must clear `deferred_pending_sprint_0c_disk` route statuses before those routes can be treated as calibrated.
