...
| 7 | ltx / `ltx-2.3` | i2v | none (only kind allowed) | LTX 2.3 distilled LoRA in template ± user `loras` | `ready_templates/video/ltx2_3_runexx_first_last_frame.py` | `start_image`/`end_image` widgets; prefix_video_source → `VHS_LoadVideo.video` widget per §3A | None | `LoraLoaderModelOnly` chain (already in template) | BLOCKED (travel adapter wiring incomplete) | `modelCapabilities.ts:107-128` × `travel_guidance.py:65-67` (non-distilled LTX2 ⇒ `{none}` only) |
| 8 | ltx / `ltx-2.3-fast` | i2v | none | distilled-fast LoRA in template ± user `loras` | same as #7 | same as #7 | None | same as #7 | BLOCKED (travel adapter wiring incomplete) | `modelCapabilities.ts:130-160` × `travel_guidance.py:64-66` |
...
**Matrix-level disposition.** Rows 1-6 depend on the Sprint 4 NEW Wan 2.2 VACE cocktail template (§3A "Wan 2.2 VACE cocktail — Q17 verdict"). Rows 7-8 resolve to the existing `ltx2_3_runexx_first_last_frame.py` template. Rows 9-13 are BLOCKED until a real LTX control-capable path is verified for a full-length control guide; they must remain WGP-only/blocked in reports rather than relying on a first/last-only seam. Four vestigial axes are flagged for §8A cleanup.
