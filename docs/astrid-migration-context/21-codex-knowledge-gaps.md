> **HISTORICAL GAP ANALYSIS — superseded for build work by `27-build-spec.md`. (Amended: Grok review — judged ADOPT.)** Live-production archaeology, exporter rehearsal, and every-family day-one parity are no longer journey gates. The surviving gaps are local availability, atomic completion, render compatibility, and production-shaped vertical-slice evidence.
